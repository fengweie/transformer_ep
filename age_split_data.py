...
all_file_name ='/mnt/workdir/fengwei/transformer_master-master/data/validation/dec1/Single_time_dataset_1Dec.xls'
               # 'CQ_12 Dec 2021.xls'
...
# 归一化
def norm(df,x):
    _range = np.max(df[x]) - np.min(df[x])
    df[x] = (df[x] - np.min(df[x])) / _range
    return df
...
df_trans = df_trans.drop(df_trans[(df_trans['pretrt_sz_5'].isnull())].index)  # 删除的行(2160->2047)
...
df_trans = df_trans.drop(df_trans[(df_trans['fam_hx'].isnull())].index)  # 删除的行(2160->2047)
df_trans['fam_hx'] = to_binary(df_trans['fam_hx'],dict(Y=1, N=0))
df_trans = df_trans.drop(df_trans[(df_trans['febrile'].isnull())].index)  # 删除的行(2160->2047)
...
df_trans = df_trans.drop(df_trans[(df_trans['alcohol'].isnull())].index)  # 删除的行
df_trans['alcohol'] = to_binary(df_trans['alcohol'],dict(Y=1, N=0))
df_trans = df_trans.drop(df_trans[(df_trans['cvd'].isnull())].index)  # 删除的行
df_trans['cvd'] = to_binary(df_trans['cvd'],dict(Y=1, N=0))
df_trans = df_trans.drop(df_trans[(df_trans['psy'].isnull())].index)  # 删除的行
df_trans['psy'] = to_binary(df_trans['psy'],dict(Y=1, N=0))
df_trans = df_trans.drop(df_trans[(df_trans['ld'].isnull())].index)  # 删除的行
...
df_trans = norm(df_trans, 'age_init')
...
print(len(df_trans))
csv_name = '/mnt/workdir/fengwei/transformer_master-master/data/validation/age_no_three/all_single_all_cols_v8.csv'
