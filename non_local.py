...
class _NonLocalBlockND(nn.Module):
    def __init__(self, in_channels, inter_channels=None, dimension=3, sub_sample=False, bn_layer=True):
        ...
        self.attn_dropout = nn.Dropout(0.03) ##########0.3 is best
    def forward(self, x, return_nl_map=False,first_layer=False):
        """
        :param x: (b, c, t, h, w)
        ...
        # print(x.shape)
        if first_layer:
            f = torch.matmul(x.permute(0, 2, 1), x)
        else:
            f = torch.matmul(theta_x, phi_x)
        f_div_C = F.softmax(f, dim=-1)
        f_div_C = self.attn_dropout(f_div_C)
